    ...
    while True:
        ...
        if datos:
            for prestamo in datos:
                print(f"")
                send_email(f'El prestamo {prestamo[1]} ha vencido el {prestamo[3]}.', email_remitente, email_destinatario, 'Prestamo Vencido')

        else:
            print("No hay prestamos vencidos.")
        cursor.close()

        sleep(60)  # Esperar 60 segundos antes de volver a verificar
